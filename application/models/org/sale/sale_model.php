...
class Sale_model extends Ion_auth_model 
{
    ...
    public function get_sale_product_list($sale_order_no, $shop_id = 0)
    {
        ...
        return $this->db->select($this->tables['product_sale_order'].'.product_id,'.$this->tables['product_sale_order'].'.purchase_order_no,'.$this->tables['product_sale_order'].'.product_category1,'.$this->tables['product_sale_order'].'.product_size,'.$this->tables['product_sale_order'].'.unit_price,'.$this->tables['product_info'].'.name as product_name,'.$this->tables['product_info'].'.name')
    ...
    public function get_sale_detail($sale_order_no, $shop_id = 0)
    {
        ...
        $this->db->group_by($this->tables['stock_info'].'.purchase_order_no');
        $this->db->group_by($this->tables['stock_info'].'.product_id');
        return $this->db->select($this->tables['stock_info'].'.product_id,'.$this->tables['stock_info'].'.purchase_order_no,'.$this->tables['stock_info'].'.product_category1,'.$this->tables['stock_info'].'.product_size,'.$this->tables['stock_info'].'.sale_order_no,'.$this->tables['product_info'].'.name as product_name,'.$this->tables['users'].'.first_name,'.$this->tables['users'].'.last_name,'.$this->tables['product_sale_order'].'.unit_price,sum(stock_out)-sum(stock_in) as total_sale,'.$this->tables['customers'].'.id as customer_id')
                    ->from($this->tables['stock_info'])
                    ->join($this->tables['sale_order'], $this->tables['sale_order'].'.sale_order_no='.$this->tables['stock_info'].'.sale_order_no')
                    ->join($this->tables['product_sale_order'], $this->tables['product_sale_order'].'.purchase_order_no='.$this->tables['stock_info'].'.purchase_order_no AND '.$this->tables['product_sale_order'].'.sale_order_no='.$this->tables['stock_info'].'.sale_order_no AND '.$this->tables['product_sale_order'].'.product_id='.$this->tables['stock_info'].'.product_id')
    ...
    public function get_daily_sales($time, $product_id = 0, $shop_id = 0 )
    {
        ...
        return $this->db->select($this->tables['sale_order'].'.created_on,'.$this->tables['product_sale_order'].'.purchase_order_no,'.$this->tables['product_sale_order'].'.product_category1,'.$this->tables['product_sale_order'].'.product_size,'.$this->tables['product_info'].'.name as product_name,sum(stock_out)-sum(stock_in) as total_sale,'.$this->tables['product_sale_order'].'.unit_price as sale_unit_price,'.$this->tables['product_purchase_order'].'.unit_price as purchase_unit_price,'.$this->tables['sale_order'].'.created_by, users_entry.first_name as entry_first_name, users_entry.last_name as entry_last_name, users_salesman.first_name as salesman_first_name, users_salesman.last_name as salesman_last_name, users_customers.first_name as customer_first_name, users_customers.last_name as customer_last_name,'.$this->tables['customers'].'.card_no,'.$this->tables['customers'].'.id as customer_id,'.$this->tables['stock_info'].'.sale_order_no,'.$this->tables['product_unit_category'].'.description as category_unit')
                    ->from($this->tables['stock_info'])
                    ->join($this->tables['sale_order'], $this->tables['sale_order'].'.sale_order_no='.$this->tables['stock_info'].'.sale_order_no')
                    ->join($this->tables['product_sale_order'], $this->tables['product_sale_order'].'.purchase_order_no='.$this->tables['stock_info'].'.purchase_order_no AND '.$this->tables['product_sale_order'].'.sale_order_no='.$this->tables['stock_info'].'.sale_order_no AND '.$this->tables['product_sale_order'].'.product_id='.$this->tables['stock_info'].'.product_id')
                    ...
                    ->join($this->tables['users'].' AS users_entry', 'users_entry.id='.$this->tables['sale_order'].'.entry_by')
                    ->get();        
    }
    
    public function get_user_sales($start_time, $end_time, $user_id = 0 , $product_id = '', $shop_id = 0, $entry_user_id = 0, $offset = 0, $limit = 0)
    {
        if($shop_id == 0)
        {
            $shop_id = $this->session->userdata('shop_id');
    ...
    public function get_user_sales_by_purchase_order_no($start_time, $end_time, $purchase_order_no = '' , $shop_id = 0, $product_category1 = '', $product_size = '')
    {
        if($shop_id == 0)
        {
            $shop_id = $this->session->userdata('shop_id');
    ...
    public function get_user_sales_by_card_no($card_no = '' , $shop_id = 0, $offset = 0, $limit = 0)
    {        
        if($shop_id == 0)
        {
            $shop_id = $this->session->userdata('shop_id');
    ...
    public function get_staff_sales_by_card_no($card_no = '' , $cell = '', $staff_id = 0, $shop_id = 0, $offset = 0, $limit = 0)
    {        
        if($shop_id == 0)
        {
            $shop_id = $this->session->userdata('shop_id');
        ...
        if(!empty($cell))
        {
            $this->db->like('users_customers.phone', $cell);
        }
        if($staff_id != 0)
        {
            $this->db->where($this->tables['sale_order'].'.created_by', $staff_id);
        }
        if($limit != 0)
        {
            $this->db->offset($offset);
    ...
    public function get_user_sales_by_name($name , $shop_id = 0)
    {
        if($shop_id == 0)
        {
            $shop_id = $this->session->userdata('shop_id');
