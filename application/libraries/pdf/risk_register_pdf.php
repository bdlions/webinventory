...
class Risk_register_pdf extends PDF {
    ...
    public function load_data($data, $styles) {
        ...
        if (is_array($data)) {
            if (key($data[RISK_REGISTER]) == REPORT_REQUEST) {
                ...
                foreach ($risk_register[REPORT_REQUEST] as $key => $value) {
    ...
    function generate_pdf() {
        ...
        $this->Cell(0, 30, "List of products", 0, 1);
        ...
        foreach ($this->risk_register_rows as $value) {
            ...
            $col_data = array();
            array_push($col_data, array('id' => SRA_RISK_ID, 'cell_width' => 15, 'cell_value' => $sra_riks_id, 'cell_style' => $this->getColumnStyleById(SRA_RISK_ID)), array('id' => DATE, 'cell_width' => 30, 'cell_value' => $date, 'cell_style' => $this->getColumnStyleById(DATE)), array('id' => SOURCE_REGISTER, 'cell_width' => 25, 'cell_value' => $source_register, 'cell_style' => $this->getColumnStyleById(SOURCE_REGISTER)), array('id' => PRACTICE_AREA, 'cell_width' => 35, 'cell_value' => $practice_area, 'cell_style' => $this->getColumnStyleById(PRACTICE_AREA)), array('id' => TRIGGER_EVENT, 'cell_width' => 45, 'cell_value' => $trigger_event, 'cell_style' => $this->getColumnStyleById(TRIGGER_EVENT)), array('id' => COMPLIANCE_OFFICER, 'cell_width' => 35, 'cell_value' => $compliance_officer, 'cell_style' => $this->getColumnStyleById(COMPLIANCE_OFFICER)), array('id' => ACTION_TAKEN, 'cell_width' => 50, 'cell_value' => $action_taken, 'cell_style' => $this->getColumnStyleById(ACTION_TAKEN)), array('id' => COMMENTS, 'cell_width' => 40, 'cell_value' => $comments, 'cell_style' => $this->getColumnStyleById(COMMENTS)));
    ...
    function Header() {
        ...
        $this->Image(base_url() . 'resources/pdf/images/' . $this->header->src, $this->header->x, $this->header->y, $this->header->width);
        $this->Ln();
    }
